			...
			function tophoto_cb(arg,data)
				if data.content_.sticker_ then
					...
					if file_exi(tostring(name), tostring(apath), tostring(pasvand)) then
						...
						tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, pfile, "@TeleStarTeam", dl_cb, nil)
					else
...
--------------------------------
	if matches[1]:lower() == "photo" or matches[1]:lower() == "تبدیل به عکس" then 
		...
		tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, file, "@TeleStarTeam", dl_cb, nil)
	end
...
if not lang then
helpfun = [[
_Star Queen Fun Help Commands:_
